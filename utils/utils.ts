...
export async function myAnchorProgram(provider: anchor.Provider, keypath: string): Promise<anchor.Program> {
...
export async function loadSVMSwitchboardProgram(provider: anchor.Provider): Promise<anchor.Program> {
...
export function getUserGuessFromCommandLine(): boolean {
  ...
  if (!userGuessInput) {
    userGuessInput = reader.question("It is now time to make your prediction: Heads or tails... ").trim().toLowerCase();
  }
